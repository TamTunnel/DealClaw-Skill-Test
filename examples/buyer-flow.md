...
PI_ID=$(curl -s -X GET "https://api.dealclaw.net/api/deals/$DEAL_ID/download" \
  -H "Authorization: Bearer $TOKEN" | grep -o '"paymentIntentId":"[^"]*"' | cut -d'"' -f4)

# 3. Confirm the payment intent
python scripts/confirm_mpp.py "$PI_ID" "$TOKEN" "https://api.dealclaw.net"
...
curl -s -X GET "https://api.dealclaw.net/api/deals/$DEAL_ID/download" \
